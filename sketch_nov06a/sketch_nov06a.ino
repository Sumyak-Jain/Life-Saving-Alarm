...
const char* ssid = "ABC"; //Your Network SSID
const char* password = "12345678911"; //Your Network Password
int val;
...
unsigned long time1;
unsigned long myChannelNumber = 1220585 ; //Your Channel Number (Without Brackets)
const char * myWriteAPIKey = "Q6IVPLEKDSUWPQBF"; //Your Write API Key
...
void loop()
{
 int flag=1;
  ...
  delay(10);
  if(analogRead(PulseSensorpin)>=140 || analogRead(PulseSensorpin)<=55)
  {
  while(flag==1)
  {
    time1=millis();
...
int get_http(String state)
{
   ...
   http.begin("http://maker.ifttt.com/trigger/Pulse/with/key/fnaGeMWtqDtLb0fR7xZXWVu1ItGam_P13cdnqCPzo2g="+state); //HTTP
